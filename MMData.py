    ...
    def update_data(self, df, config):
        ...
        for i, series in enumerate(config['series']):
            ...
            if raw_key in df.columns:
                self.ax.plot(df['Relative_Time_s'], df[raw_key], color=color, lw=0.8, ls='--', alpha=0.4, label=f'{label_base} (Raw)')

            if lpf_key in df.columns:
                self.ax.plot(df['Relative_Time_s'], df[lpf_key], color=color, lw=1.5, label=f'{label_base} (5Hz LPF)')
