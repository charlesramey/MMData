"""
Tunnel-specific video-to-IMU alignment, V2.
...
This replaces the V1 approach of matching the first-leap frame to the first
significant IMU stride peak.
"""
...
MOTION_ONSET_CSV = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                'Tunnel Motion Onset Frames.csv')


class TunnelAlignerV2(ObstacleAligner):
    """Align a tunnel repetition using the motion-onset-to-trim-start heuristic.

    Parameters
    ----------
    data_dir : str
        Path to a repetition folder (e.g. 720sync/Arya_Tunnel_1/).
    motion_onset_frame : int or None
        Video frame number where the dog begins directed forward locomotion.
        If None, attempts to read from Tunnel Motion Onset Frames.csv.
    prefer_trimmed : bool
    ...
    def _detect_motion_onset_frame(self):
        """Look up the motion onset frame from Tunnel Motion Onset Frames.csv."""
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Usage: python tunnel_aligner_v2.py <data_dir> [motion_onset_frame]")
        sys.exit(1)

    data_dir = sys.argv[1]
    onset = int(sys.argv[2]) if len(sys.argv) > 2 else None
    aligner = TunnelAlignerV2(data_dir, motion_onset_frame=onset)
